...
def make_mask(row_id, df):
    ...
    comb = masks[:, :, 0] + masks[:, :, 1] + masks[:, :, 2] + masks[:, :, 3]
    comb=comb.astype(np.uint8)
    # print(f'comb {np.unique(comb)}')
    # plt.imshow(comb)
    # plt.show()
    return fname, comb
# *****************create a 4d mask of 4 error classes in an array*********************
...
class SteelDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        print(f'earlier {np.unique(mask)}')
        augmented = self.transforms(image=img, mask=mask)

        img = augmented['image']
        mask = augmented['mask']  # 1x256x1600x4
        # mask = mask[0].permute(2, 0, 1)  # 1x4x256x1600
        print(f'later {np.unique(mask)}')
        return img, mask
...
class Meter:
    ...
    def update(self, targets, outputs):
        ...
        print (f'unique {np.unique(targets)}')
...
class Trainer(object):
    ...
    def forward(self, images, targets):
        ...
        masks = targets.type('torch.LongTensor')
        masks = masks.to(self.device)
        masks=masks.view(-1,)
        outputs = self.net(images)
        outputs = outputs.transpose(1, 2).transpose(2, 3).contiguous().view(-1, 4)
        outputs=outputs.view(-1,1)
        masks = masks.view(-1)
        loss = F.cross_entropy(outputs, masks,size_average=True)       #calculate the binary cross enrtopy from logits.
        return loss, outputs
...
sample_submission_path = '/media/hpc2/New_Volume/shashank/steel/sample_submission.csv'
train_df_path = '/media/hpc2/New_Volume/shashank/steel/train.csv'
data_folder = "/media/hpc2/New_Volume/shashank/steel"
test_data_folder = "/media/hpc2/New_Volume/shashank/steel/test_images"
